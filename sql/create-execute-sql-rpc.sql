-- 创建 execute_sql RPC 函数（绕过 PostgREST schema cache 直接操作数据库）
-- 执行方法：在 Supabase Dashboard → SQL Editor 中粘贴并运行
-- 只需执行一次，之后所有页面刷新即可生效

-- 1. 创建 execute_sql 函数（仅当不存在时）
...
-- 3. 刷新 PostgREST schema cache（让 REST API 也能看到 category 列）
NOTIFY pgrst, 'reload schema';

-- 4. 验证：确认 assets 表有 category 列
